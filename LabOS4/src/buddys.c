...
typedef struct Allocator {
    void *memory;       // Указатель на область памяти
    size_t size;        // Размер всей памяти
    uint8_t *bitmap;    // Таблица занятости блоков
    size_t block_size;  // Минимальный размер блока
} Allocator;
